    ...
    if(err){
      ...
      server
        ...
        .then((cliente)=>{
            ...
            console.log("SOCKET: mensaje enviado a "+cliente.usuario.username+" id: "+cliente.usuario.id+" socketId:"+cliente.socket.id);
        }).catch((err)=>{
          console.log("error cliente",err);
            server
              ...
              .then((usuario)=>{      
                ...
                var push = new OneSignal.Notification({"contents": {"en" : texto,"es" : texto}});
                push.setTargetDevices([usuario.deviceId]);
